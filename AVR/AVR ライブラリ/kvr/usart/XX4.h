// ------------------------------------------------------------------
// kvr / KUT AVR Library
// USART for XX4 Library / "kvr/usart/XX4.h" / v1.1
// Author: MIZUNE Pine
// ------------------------------------------------------------------
//
	...
	)
{
	...
	*usart_port[num][KVR_USART_UBRRL] = (uint8_t)ubrr;
	...
	// 送信許可 & 受信許可
	*usart_port[num][KVR_USART_UCSRB] = _BV(TXEN0) | _BV(RXEN0);
...
{
	// 受信するまで待機
	loop_until_bit_is_set(*usart_port[num][KVR_USART_UCSRA], RXC0);
	
	// 受信
	return *usart_port[num][KVR_USART_UDR];
}
...
{
	// 受信許可 & 受信割り込み許可
	*usart_port[num][KVR_USART_UCSRB] |= _BV(RXCIE0);
