...
void EEPROM_w(unsigned int uiAddress, unsigned char ucData)
{
...
unsigned char EEPROM_r(unsigned int uiAddress)
{
	 while(EECR & (1<<EEPE));	 /* 以前のEEPROMﾌﾟﾛｸﾞﾗﾐﾝｸﾞ完了まで待機 */
	 EEAR = uiAddress;	 /* EEPROMｱﾄﾞﾚｽ設定 */
...
void fw_straight(){
		...
		//f = img_boad2();
		//b = img_boad1();
		EEPROM_w(2,(int)img_boad2());
		EEPROM_w(1,(int)img_boad1());
		lt_back(place((int)EEPROM_r(2),(int)EEPROM_r(1)));
}
