...
int main(){
	...
	n = 0;
    //usart_init(&USARTE0, 4800); // USART の初期設定 (ポートE; ボーレート 9600)
	PORTA.DIR = 0x00;
	while(1){
		if(f_tsen()==0){
			//brake();
			while(f_tsen()==0)_delay_ms(10);
		}
		else if(f_tsen()==1){
			turn_right(0xff);
			while(f_tsen()==1)_delay_ms(10);
		}
		else if(f_tsen()==2){
			turn_left(0xff);
			while(f_tsen()==2)_delay_ms(10);
		}
		else if(f_tsen()==3){
			straight(0xff);
			while(f_tsen()==3)_delay_ms(10);
		}
		else if(f_tsen()==4){
			dis();
			while(f_tsen()==4){
