...
void lt_left(int c){
	...
	for(int cnt = 0;cnt <= c;c++){
		...
		while(1){
			if((f_right3 > 310) || (f_left3 > 310) || (f_right2 > 310) || (f_left2 > 310) || (f_left3 > 310))break;
		}
	}
	brake();
	_delay_ms(1000);
	...
	while(1){
		adc_init(10);
		if((f_left3 > 310) && (f_left2 > 310))break;
	}
}
...
void lt_right(int c,char s){
	...
	for(int cnt = 0;cnt <= c;c++){
		...
		while(1){
			if((f_right3 > s) || (f_left3 > s) || (f_right2 > s) || (f_left2 > s) || (f_left3 > s))break;
		}
	}
	brake();
	_delay_ms(1000);
	...
	while(1){
		adc_init(10);
		if((f_right3 > s) && (f_right2 > s))break;
	}
}
