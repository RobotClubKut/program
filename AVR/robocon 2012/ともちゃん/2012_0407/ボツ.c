				...
				if(IIR(f_right1,11) >= 3){
					if(IIR(f_left1,9) >= 3){
						if(IIR(f_center,10) >= 3){
							...
							straight(s);
							while((IIR(f_center,10) >= 3) && (IIR(f_right2,12) >= 3) && (IIR(f_right1,11) >= 3) && (IIR(f_left2,8) >= 3) && (IIR(f_left1,9) >= 3)){}
							_delay_ms(t);
...
void lt_right(int c,char s){
	...
	for(int cnt = 0;cnt <= c;c++){
		...
		while(1){
			if((f_right3 >= 200+s) || (f_left3 >= 180+s) || (f_right2 >= 260+s) || (f_left2 >= 240+s))break;
		}
	}
	brake();
	_delay_ms(1000);
	...
	while(1){
		adc_init(10);
		if((f_left3 >= 200+s) && (f_left2 >= 255+s))break;
	}
}
...
void fw_go2(int n,int t,unsigned char s){	
	unsigned char p = s;
	int cnt = 0;
