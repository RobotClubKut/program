...
void go(int n,int t,int s,int se){	
	int p=s;
	int cnt = 0;
	while(cnt < n){
		if(IIR(f_right2+se,12) >= 3){
			if(IIR(f_left2+se,8) >= 3){
				if(IIR(f_right1+se,11) >= 3){
					if(IIR(f_left1+se,9) >= 3){
						if(IIR(f_center+se,10) >= 3){
							cnt++;					//フォトトランジスタ5つ反応でcntを+1
							straight(s);
							while((IIR(f_center+se,10) >= 3) && (IIR(f_right2+se,12) >= 3) && (IIR(f_right1+se,11) >= 3) && (IIR(f_left2+se,8) >= 3) && (IIR(f_left1+se,9) >= 3)){}
							_delay_ms(t);
			...
			if(IIR(r_right2,5) >= 3 && IIR(r_right2,5) <= 4){
				if(IIR(r_left3,0) >= 3 && IIR(r_left3,0) <= 4){
					if(IIR(r_left2,1) >= 3 && IIR(r_left2,1) <= 4){
						...
						fw_straight2(s);
						while((IIR(r_center,3) >= 3) && (IIR(r_right3,6) >= 3) && (IIR(r_right1,4) >= 3) && (IIR(r_left3,0) >= 3) && (IIR(r_left1,2) >= 3) && (IIR(r_center,3) <= 4) && (IIR(r_right3,6) <= 4) && (IIR(r_right1,4) <= 4) && (IIR(r_left3,0) <= 4) && (IIR(r_left1,2) <= 4)){}
						if(cnt == 1){
							//t+20?
							_delay_ms(t + 10);
						}
