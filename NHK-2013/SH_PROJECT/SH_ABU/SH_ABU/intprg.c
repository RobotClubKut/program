...
//extern signed int R,L;
extern signed int X,Y,D;
extern char CMT0F;
...
// 64 Interrupt IRQ0
void INT_IRQ0(void){		//ホイールL
	INTC.IRQSR.BIT.IRQ0F=0; //フラグクリア
	if(RE0B!=RE0A){	//正転
//		L+=1;
		D-=114286;
		if(D<=-180*1000000)
			D+=360*1000000;
		X-=(628.3184266)*sin(((double)(D+57143)*M_PI)/(double)180/1000000);
		Y+=(628.3184266)*cos(((double)(D+57143)*M_PI)/(double)180/1000000);
	}
	else{	//逆転
//		L-=1;
		D+=114286;
...
// 67 Interrupt IRQ3
void INT_IRQ3(void){		//ホイールR
	INTC.IRQSR.BIT.IRQ3F=0; //フラグクリア
