...
void LIN_1_IntClk_Start(void) 
{
    ...
    LIN_1_IntClk_CLKEN |= LIN_1_IntClk_CLKEN_MASK;
	LIN_1_IntClk_CLKSTBY |= LIN_1_IntClk_CLKSTBY_MASK;
}
...
void LIN_1_IntClk_Stop(void) 
{
    ...
    LIN_1_IntClk_CLKEN &= (uint8)(~LIN_1_IntClk_CLKEN_MASK);
	LIN_1_IntClk_CLKSTBY &= (uint8)(~LIN_1_IntClk_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: LIN_1_IntClk_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((LIN_1_IntClk_CLKEN & LIN_1_IntClk_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = LIN_1_IntClk_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = LIN_1_IntClk_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* LIN_1_IntClk__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(LIN_1_IntClk_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(LIN_1_IntClk_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: LIN_1_IntClk_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: LIN_1_IntClk_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = LIN_1_IntClk_GetSourceRegister();
    uint16 oldDivider = LIN_1_IntClk_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = LIN_1_IntClk_CLKEN & LIN_1_IntClk_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(LIN_1_IntClk_DIV_PTR, clkDivider);
                LIN_1_IntClk_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                LIN_1_IntClk_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(LIN_1_IntClk_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((LIN_1_IntClk_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
{
    return CY_GET_REG16(LIN_1_IntClk_DIV_PTR);
}


/*******************************************************************************
* Function Name: LIN_1_IntClk_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    LIN_1_IntClk_MOD_SRC |= modeBitMask & (uint8)LIN_1_IntClk_MODE_MASK;
}


/*******************************************************************************
* Function Name: LIN_1_IntClk_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    LIN_1_IntClk_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(LIN_1_IntClk_MODE_MASK));
}


/*******************************************************************************
* Function Name: LIN_1_IntClk_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return LIN_1_IntClk_MOD_SRC & (uint8)(LIN_1_IntClk_MODE_MASK);
}


/*******************************************************************************
* Function Name: LIN_1_IntClk_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = LIN_1_IntClk_GetDividerRegister();
    uint8 oldSrc = LIN_1_IntClk_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        LIN_1_IntClk_MOD_SRC |= CYCLK_SSS;
        LIN_1_IntClk_MOD_SRC =
            (LIN_1_IntClk_MOD_SRC & (uint8)(~LIN_1_IntClk_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: LIN_1_IntClk_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return LIN_1_IntClk_MOD_SRC & LIN_1_IntClk_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: LIN_1_IntClk_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    LIN_1_IntClk_PHASE = clkPhase & LIN_1_IntClk_PHASE_MASK;
}


/*******************************************************************************
* Function Name: LIN_1_IntClk_GetPhase
********************************************************************************
*
* Summary:
...
{
    return LIN_1_IntClk_PHASE & LIN_1_IntClk_PHASE_MASK;
}
