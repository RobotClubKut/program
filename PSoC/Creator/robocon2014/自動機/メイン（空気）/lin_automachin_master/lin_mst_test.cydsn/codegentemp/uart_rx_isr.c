...
CY_ISR(uart_rx_isr_Interrupt)
{
    ...
    /* PSoC3 ES1, ES2 RTC ISR PATCH  */ 
        ...
        #if((CYDEV_CHIP_REVISION_USED <= CYDEV_CHIP_REVISION_3A_ES2) && (uart_rx_isr__ES2_PATCH ))      
            uart_rx_isr_ISR_PATCH();
...
{
    CY_SET_REG16(uart_rx_isr_INTC_VECTOR, (uint16) address);
}
...
{
    return (cyisraddress) CY_GET_REG16(uart_rx_isr_INTC_VECTOR);
}
...
uint8 uart_rx_isr_GetState(void) 
{
    ...
    return ((*uart_rx_isr_INTC_SET_EN & (uint8)uart_rx_isr__INTC_MASK) != 0u) ? 1u:0u;
}
